...
class Owner_data {
  late String
      id; // late keyword added in order to initialize the variable later
  late final String name;
  late final List<String>? plist;
  late final int joined_at; // Year of registration
  late List<String> chat_id_list;
  var image;
  var imageUrl;

  Owner_data(this.name, this.id,
      {this.plist = const [], int? joined_at, this.chat_id_list = const [], this.image}) {
    // Id will be generated by firebase when first signing up.
    this.joined_at = (joined_at != null)
        ? joined_at
        : int.parse(DateFormat('yyyy').format(DateTime.now()));
  }
  void Add_Property(String new_property) {
    // This function should only be called via Firebase_functions.Add_Property (IMPORTANT)
    plist!.add(new_property);
  }

  void Remove_Property(String old_property) {
    // This function should only be called via Firebase_functions.Remove_Property (IMPORTANT)
